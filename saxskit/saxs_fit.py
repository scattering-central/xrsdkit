...
class SaxsFitter(object):
    ...
    def evaluate_as_array(self,param_keys,param_array):
        """param_keys and param_array should have been generated by self.unpack_params()"""
    ...
    def evaluate(self,params):
        ...
        #        chi2log = saxs_math.compute_chi2(
        #            np.log(I_comp[self.idx_fit]),
        #            self.logI[self.idx_fit],self.q[self.idx_fit]**2)
        #        chi2log_total += chi2log
        #    if pkey == 'spherical_normal':
        #        # weight the low-q region? or no weights at all?
        #        chi2log = saxs_math.compute_chi2(
        #            np.log(I_comp[self.idx_fit]),
        #            self.logI[self.idx_fit])
        #        #chi2log = compute_chi2(I_comp[self.idx_fit],self.logI[self.idx_fit],1./self.q)
        #        #chi2log = compute_chi2(I_comp[self.idx_fit],self.logI[self.idx_fit],self.q[::-1])
        #        chi2log_total += chi2log
    ...
    def fit(self,params,fixed_params=None,objective='chi2log'):
        ...
        fit_obj = partial(self.evaluate_as_array,x_keys)
...
def fit_spectrum(q_I,populations,params,fixed_params=None,objective='chi2log'):
    ...
    populations : dict
        Dict enumerating scatterer populations. 
        See saxs_math module documentation.
    params : dict
        Dict of scattering equation parameters (initial guess).
        See saxs_math module documentation.
